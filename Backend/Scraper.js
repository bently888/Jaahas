...
const getBlankoMenu = async () => {
  if (blankocache.timestamp && nyt() - blankocache.timestamp < 18000000)
    return blankocache.menu;
  ...
  $(".menu-item-title").each((index, element) =>
    menuItemTitles.push(element.firstChild.data)
  );
  const menuWithoutWeek = menuItemTitles.filter(
    (menu) => menu !== menuItemTitles[0]
  );
  let Weekday = "ma";
  let menuWeekArrays = {
    ma: [],
...
const getTreviMenu = async () => {
  if (trevicache.timestamp && nyt() - trevicache.timestamp < 18000000)
    return trevicache.menu;
  ...
  const dayObjectMa = menuItemTitles.ma.map((menuitem) => ({
    price: menuitem.match(priceExpression)[0],
    food: menuitem.replace(foodExpression, ""),
  }));
...
const getFontanaMenu = async () => {
  if (fontanacache.timestamp && nyt() - fontanacache.timestamp < 18000000)
    return fontanacache.menu;
  const axiosResponse = await axios.get("https://www.fontana.fi/lunch/");

  const $ = await cheerio.load(axiosResponse.data);
  let menuItemTitles = { ma: [], ti: [], ke: [], to: [], pe: [] };
  let menuItemTitles2 = { ma: [], ti: [], ke: [], to: [], pe: [] };
  menuItemTitles.ma = handleFontanaFoodRow(
    $("#fontana-ma b, #fontana-ma span")
  );
  menuItemTitles.ti = handleFontanaFoodRow(
    $("#fontana-ti b, #fontana-ti span")
  );
  menuItemTitles.ke = handleFontanaFoodRow(
    $("#fontana-ke b, #fontana-ke span")
  );
  menuItemTitles.to = handleFontanaFoodRow(
    $("#fontana-to b, #fontana-to span")
  );
  menuItemTitles.pe = handleFontanaFoodRow(
    $("#fontana-pe b, #fontana-pe span")
  );
...
app.get("/", (req, res) => {
  if (req.protocol !== "https") res.redirect("https://" + req.hostname);
...
app.post("/join", async function (req, res) {
  ...
  const indexOfOldTrain = reservations.indexOf(
    (foodTrainItem) =>
      foodTrainItem.resta === joinRestaurant && foodTrainItem.time === joinTime
  );
  reservations = reservations.map((foodTrainItem) => {
    if (
      foodTrainItem.resta === joinRestaurant &&
      foodTrainItem.time === joinTime &&
    ...
    if (
      ...
      return {
        ...foodTrainItem,
        participants: [...foodTrainItem.participants, req.body.user],
      };
    else return foodTrainItem;
  });
  res.send(reservations);
});
