...
const listOfRestaurants = [
  ...
  { name: "Di Trevi", url: "/ditrevi", lunchUrl: "https://ditrevi.fi/lounas/" },
  {
    name: "Fontana",
    url: "/fontana",
    lunchUrl: "https://www.fontana.fi/lunch/",
  },
  { name: "Tintå", url: "/tinta", lunchUrl: "https://www.tinta.fi/lounas" },
  {
    name: "Dennis",
    url: "na",
    lunchUrl: "https://dennis.fi/pizzeria-ravintolat/turku-linnankatu/",
  },
  {
    name: "Niska",
    url: "na",
    lunchUrl: "https://www.niskaturku.com/fi/Lounas",
  },
  { name: "Kawaii", url: "na", lunchUrl: "https://www.ravintolakawaii.com/" },
];

export const alertTimeSplit = (timeOfAlert) => {
  const splitAlertTime = timeOfAlert.split(":");
  const secondsAlert = splitAlertTime[0] * 60 * 60 + splitAlertTime[1] * 60;
  return secondsAlert;
};
...
function App() {
  ...
  useEffect(() => {
    setFilteredRestaurantData(
      restaurantData.filter((restaurant) =>
        restaurant.name.toLowerCase().includes(newFilter.toLowerCase())
      )
    ...
    listOfRestaurants.forEach((restaurant) => {
      if (restaurant.url !== "na")
        axios
          .get(restaurant.url)
          .then((response) =>
            setRestaurantData((oldState) => [
              ...oldState,
              {
                ...response,
                name: restaurant.name,
                lunchUrl: restaurant.lunchUrl,
              },
            ])
          );
      else
        setRestaurantData((oldState) => [
          ...oldState,
          { name: restaurant.name, lunchUrl: restaurant.lunchUrl },
        ]);
    });
    const savedUser = localStorage.getItem("userData");
    axios.get("/reservations").then((reservation) => {
      setFoodTrain(reservation.data);
      const userTrain = reservation.data.find((train) =>
        train.participants.includes(savedUser)
      );
      if (userTrain) setTrainLeaveTime(userTrain.time);
    });
    setUser(savedUser || "");
  }, []);

  useEffect(() => {
    const userTrain = foodTrain.find((train) =>
      train.participants.includes(user)
    );
    if (userTrain) setTrainLeaveTime(userTrain.time);
    else setTrainLeaveTime(null);
  }, [user, foodTrain]);
  ...
  return (
    <div className="App">
      ...
      <h1 className="main-title">Jaahas</h1>
      {/* oikealla */}
      <button
        className="show-tomorrow"
        disabled={isFriday}
      ...
      {/* vasemmalla */}
      <p className="set-filter">
        <input onChange={filterOnChange} type="search" />
        filter restaurants
      </p>
      {/* keskellä */}
      {trainLeaveTime ? (
        <p className="alert-text">
          Alert will be shown 3 minutes before {trainLeaveTime}
        </p>
      ) : (
      ...
      <Restaurants
        ...
        allowReservations={user.length > 0}
