...
const joinTrain = (clickedItem, foodTrain, user, setFoodTrain) => {
    //Tähän nimen syöttö
  ...
  const deleteName = (user, foodTrain, setFoodTrain) => {
...
function FoodTrains(props) {
  return (
    <div className="times-chosen">
        ...
        <button onClick={() => joinTrain(res, props.foodTrain, props.user, props.setFoodTrain)} 
        ...
        <button onClick={() => deleteName(props.user, props.foodTrain, props.setFoodTrain)} disabled={!props.foodTrain.some(train => train.participants.includes(props.user))}>delete</button>
    </div>
  );
}
